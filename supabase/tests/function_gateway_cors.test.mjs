...
try {
  ...
  try {
    for (const name of ['generate-draft', 'review-draft', 'run-schedules']) await verifyFunction(name, keys.anon, user.accessToken);
  } finally {
    await deleteTestUser(user.id, keys.service);
  }
  console.log('Actual local Supabase gateway CORS/auth probes passed for 3 functions.');
} finally {
  stopServer(child);
