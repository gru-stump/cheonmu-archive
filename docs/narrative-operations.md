...
생성 대기열 worker와 소유자 전용 접속 트리거는 로컬에서 구현·검증되었습니다. 관리자는
**오늘 > 접속 이야기 만들기**를 눌러 모델과 최대 원화 비용을 먼저 확인한 뒤 요청합니다.
브라우저는 확인한 최대 비용과 현재 소유자 bearer만 보내며, owner/job/schedule/provider/model 값은
서버와 데이터베이스가 결정합니다. 동일 출처 서버가 `run-schedules`의 `action: access` 명령을 만들고, 데이터베이스가
최소 간격·서울 일일 한도·가격 유효성·예산·자동 일정 허용을 판정합니다. 공개 아카이브는 이
소유자 권한이나 호출 경로를 사용하지 않습니다. 수동 생성 허용과 자동 일정 허용은 서로 다른
서버 정책이며, provider 선택은 두 정책과 독립적으로 유지됩니다. 로컬 캐너리는
`수동 생성 허용=켜짐`, `자동 일정 허용=꺼짐`, `fake-local-provider=활성`으로 고정됩니다.
호스팅 환경의 access 경로와 worker 증거가 있기 전에는 라이브 캐너리와 생산 일정을 활성화하지
않습니다. 대기열 호출 성공만으로 실제 초안 생성 완료를 뜻하지 않기 때문입니다.
...
관리자 화면에서는 API 키를 먼저 연결한 뒤 `모델 불러오기`를 눌러 선택 목록을 봅니다.
목록을 불러오지 못하면 마지막으로 검증한 안전 목록이 `확인 필요` 상태로 표시되며, 임의의
모델 이름을 추측해 입력하지 않습니다. 입력·출력 단가와 토큰 한도는 기본 화면에서 숨기고
`고급 설정`에서만 확인합니다. 이 목록과 가격 자료의 마지막 검증일은 **2026-08-16**이며,
제공자 공식 가격표나 모델 사용 가능 여부가 바뀌면 코드의 카탈로그와 검증일을 함께 갱신합니다.

OpenAI 또는 Anthropic API 키를 삭제하면 수동 생성과 자동 일정이 모두 즉시 멈춥니다.
삭제 뒤 화면이 `연결 안 됨`이고 두 정책이 모두 꺼졌는지 확인한 후에만 새 키를 연결합니다.
GitHub 키 삭제는 이야기 생성을 멈추지는 않지만 새 게시를 할 수 없게 합니다.
...
제12절의 캐너리가 통과하고 호스팅 환경의 access 호출 경로가 검증되기 전에는
아래 순서를 실행하지 않습니다.

1. **일간만 활성화**: 선택한 서울 시각, 최소 간격, `short_dialogue`로 시작합니다.
2. 24시간 뒤 job/draft 상태, provider token, ledger 정산, 예산 상태, 연속성 경고를 검토합니다.
3. **접속 시 생산**: 페이지 로드 자동 호출이 아닌 **오늘 > 접속 이야기 만들기**로만 요청합니다.
   화면의 모델과 최대 원화 비용을 확인하고 승인한 뒤, `이야기 생성 현황`에서 정확한 한국 시각과
   `완료`를 확인합니다. 기술 정보는 문제 조사 때만 펼치며, 서버의 최소 간격·서울 일일 호출 한도로
   중복이 차단되는지도 확인합니다.
