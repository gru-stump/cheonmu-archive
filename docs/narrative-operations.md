...
**오늘 > 접속 생성 요청**을 명시적으로 눌러 body 없는 `POST /api/narrative/access`를 호출하며,
브라우저는 현재 소유자 bearer 외의 owner/job/schedule/provider/pricing/mode/kind/idempotency 값을
보내지 않습니다. 동일 출처 서버가 `run-schedules`의 `action: access` 명령을 만들고, 데이터베이스가
...
3. **접속 시 생산**: 페이지 로드 자동 호출이 아닌 **오늘 > 접속 생성 요청**으로만 요청하고,
   서버의 최소 간격·서울 일일 호출 한도로 중복이 차단되는지 확인한 뒤 켭니다.
