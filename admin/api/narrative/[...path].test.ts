...
describe('deployed narrative API adapter', () => {
  ...
  it('preserves an absent POST body for the owner access trigger', async () => {
    vi.stubEnv('SUPABASE_URL', 'https://db.example.test');
    vi.stubEnv('SUPABASE_ANON_KEY', 'anon');
    const upstream: typeof globalThis.fetch = vi.fn(async (input) => {
      ...
      return Response.json({
        ...
        payload: { kind: 'short_dialogue', source: 'access' },
      }, { status: 202 });
    });
    ...
    await handler({
      method: 'POST', url: '/api/narrative/access', body: undefined,
      headers: { host: 'admin.example.test', authorization: 'Bearer owner-token' },
    }, response);

    expect(sent).toEqual({ status: 202, body: JSON.stringify({ id: 'persisted-job', scheduledFor: '2026-08-16T09:00:00Z' }) });
