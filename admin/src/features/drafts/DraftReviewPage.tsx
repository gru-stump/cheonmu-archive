...
export function DraftReviewPage({ api, draftId }: { api: NarrativeApi; draftId: string }) {
  ...
  const load = async () => { setLoadError(false); const value = await api.getDraft(draftId); setDetail(value); setManualText(value.latestVersion.content.body); setStale(false); };
  ...
  const closeDialog = () => { setDialog(null); queueMicrotask(() => triggerRef.current?.focus()); };
  const openDialog = (kind: Exclude<DialogKind, null>, event: React.MouseEvent<HTMLButtonElement>) => { triggerRef.current = event.currentTarget; setDialog(kind); setMessage(null); };
  ...
  const review = async (action: ReviewInput['action'], reviewReason?: string) => {
    ...
    try {
      ...
      setDetail({ ...detail, status: result.status }); setMessage('검토 결과를 저장했습니다.'); closeDialog();
  ...
  const revise = async (event: FormEvent) => {
    event.preventDefault(); setMessage(null);
    const estimated = estimateRevisionCost(detail, maxTokens);
    if (!selectedText.trim() || !instruction.trim() || maxTokens < 1 || !costConfirmed) { setMessage('구절, 수정 지시, 최대 토큰과 비용 확인이 필요합니다.'); return; }
    try {
      await api.generate({ draftId, expectedVersionId: detail.latestVersionId, expectedState: detail.status === 'generated' ? 'generated' : 'reviewing', mode: 'revise_selection', kind: detail.kind, revision: { selectedText: selectedText.trim(), instruction: instruction.trim() }, requestedMaxOutputTokens: maxTokens, maximumCostConfirmed: true, confirmedMaximumCostMicros: estimated });
      setMessage('부분 수정 결과를 새 버전으로 생성했습니다.'); closeDialog(); await load();
  ...
  return (
    <article aria-labelledby="draft-title">
      ...
      {message && <p role={stale ? 'alert' : 'status'}>{message}</p>}
      {stale && <button type="button" onClick={() => void load()}>새로 불러오기</button>}
      ...
      <div className="review-actions" aria-label="초안 작업">
        ...
        {!blocked && detail.status !== 'archived' && <button type="button" onClick={() => void archive()}>보관</button>}
        {!blocked && detail.status === 'publish_failed' && <button type="button" onClick={() => void retryPublish()}>게시 재시도</button>}
      </div>
      {dialog === 'manual' && <Modal title="직접 수정" onClose={closeDialog}><form onSubmit={saveManual}><label htmlFor="manual-body">최종 본문</label><textarea id="manual-body" rows={12} value={manualText} onChange={(event) => setManualText(event.target.value)} required /><button type="submit">새 버전 저장</button></form></Modal>}
      {dialog === 'revision' && <Modal title="부분 AI 수정" onClose={closeDialog}><form onSubmit={revise}><label htmlFor="selected-text">선택한 구절</label><textarea id="selected-text" value={selectedText} onChange={(event) => setSelectedText(event.target.value)} required /><label htmlFor="revision-instruction">수정 지시</label><textarea id="revision-instruction" value={instruction} onChange={(event) => setInstruction(event.target.value)} required /><label htmlFor="max-tokens">최대 출력 토큰</label><input id="max-tokens" type="number" min="1" max={detail.revisionPricing?.maximumRevisionOutputTokens ?? 4096} value={maxTokens} onChange={(event) => { setMaxTokens(Number(event.target.value)); setCostConfirmed(false); }} required /><p>예상 최대 비용: {estimateRevisionCost(detail, maxTokens).toLocaleString('en-US')} μUSD</p><label><input type="checkbox" checked={costConfirmed} onChange={(event) => setCostConfirmed(event.target.checked)} /> 최대 비용을 확인했습니다</label><button type="submit">새 버전 생성</button></form></Modal>}
      {dialog === 'reject' && <Modal title="거절" onClose={closeDialog}><form onSubmit={(event) => { event.preventDefault(); void review('reject', reason); }}><label htmlFor="reject-reason">거절 사유</label><textarea id="reject-reason" value={reason} onChange={(event) => setReason(event.target.value)} required /><button type="submit">거절 확정</button></form></Modal>}
    </article>
  );
}
