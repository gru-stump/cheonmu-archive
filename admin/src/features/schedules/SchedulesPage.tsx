...
function ScheduleForm({ api, schedule, readOnly, onSaved, onMessage }: { api: NarrativeApi; schedule: NarrativeSchedule; readOnly: boolean; onSaved(): Promise<void>; onMessage(message: string): void }) {
  ...
  return <article className="schedule-row">
    ...
    <form className="schedule-form" onSubmit={save}>
      ...
      <input id={`schedule-key-${schedule.id}`} value={draft.scheduleKey} disabled={readOnly} onChange={(event) => setDraft({ ...draft, scheduleKey: event.target.value })} required /></div>
      ...
      <div className="schedule-field schedule-field--toggle"><label className="control-target"><input type="checkbox" checked={draft.enabled} disabled={readOnly} onChange={(event) => setDraft({ ...draft, enabled: event.target.checked })} /> 활성</label></div>
