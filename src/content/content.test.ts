...
describe('initial Cheonmu archive content', () => {
  ...
  it('includes every sourced appellation and its relationship change note', () => {
    ...
    expect(relationshipBody).not.toContain('- 천령 선생');
    expect(relationshipBody).not.toContain('- 선생님');
    expect(relationshipBody).toContain('## 관계가 깊어진 뒤의 변화');
    expect(relationshipBody).toContain('심각하거나 감정이 흔들리는 순간에는 ‘무영’이라고만 부른다');
    expect(relationshipBody).toContain('감정이 크게 흔들릴 때는 ‘천령’이라고만 부른다');
    expect(relationshipBody).toContain('관계 단계와 무관하게 ‘천령 의료관님’을 기본으로 쓴다');
    expect(relationshipBody).toContain('‘천령 선생’과 ‘선생님’은 사용하지 않는다');
  });
});
